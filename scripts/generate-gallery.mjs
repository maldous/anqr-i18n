...
async function waitForAnimationReady(page, itemId) {
  ...
  console.log(`    [${itemId}] Waiting for animation frames to generate (max ${ANIMATED_MAX_TIMEOUT / 60000} minutes)...`)
  ...
  while (Date.now() - startTime < ANIMATED_MAX_TIMEOUT) {
    ...
    if (state) {
      ...
      if (state.isAnimationReady && state.frameCount > 1) {
        ...
        console.log(`    [${itemId}] Animation ready: ${state.frameCount} frames generated in ${elapsed}s`)
      ...
      if (elapsed - lastLogTime >= 30000) {
        const status = state.isLoading ? 'loading' : (state.frameCount > 0 ? `${state.frameCount} frames` : 'waiting')
  ...
  console.log(`    [${itemId}] Warning: Animation timeout reached after ${ANIMATED_MAX_TIMEOUT / 60000} minutes`)
  return { ready: false, frameCount: 0 }
}
...
async function captureQR(page, item) {
  ...
  if (item.isAnimated) {
    ...
    const avgDelay = frameDelays.length > 0 ? Math.round(frameDelays.reduce((a, b) => a + b, 0) / frameDelays.length) : 100
    console.log(`    [${item.id}] Captured ${framesData.length} frames with original timing (avg ${avgDelay}ms per frame)`)
