...
class ANQRApp {
  ...
  bindEvents() {
    ...
    const autoUpdateInputs = [
      'qrContent', 'typeNumber', 'moduleSize', 'margin', 'moduleGap',
      'fgColor', 'bgColor', 'moduleStyle', 'finderStyle',
      'overlayMode', 'overlayIntensity', 'logoSize',
      'animationSpeed', 'colorMode'
    ];
    ...
    // Radio buttons
    document.querySelectorAll('input[name="errorCorrection"]').forEach(radio => {
      radio.addEventListener('change', () => this.scheduleGenerate());
    });
    
    // Checkboxes
    ['transparentBg', 'preserveFinders', 'loopAnimation', 'reverseAnimation', 'invertImage'].forEach(id => {
      const el = document.getElementById(id);
      if (el) {
        el.addEventListener('change', () => this.scheduleGenerate());
      }
    ...
    // Overlay mode change
    document.getElementById('overlayMode').addEventListener('change', () => this.updateOverlayModeUI());
    ...
    document.querySelector('.modal-backdrop')?.addEventListener('click', () => this.hideShareModal());
    document.getElementById('copyDirectLink')?.addEventListener('click', () => this.copyDirectLink());
    document.getElementById('copyEmbedHtml')?.addEventListener('click', () => this.copyEmbedHtml());
    ...
    document.getElementById('outputWidth').addEventListener('change', () => {
      document.getElementById('outputHeight').value = document.getElementById('outputWidth').value;
    });
  ...
  async generate() {
    ...
    try {
      ...
      if (this.overlayFrames.length > 1) {
        ...
        document.getElementById('totalFrames').textContent = this.currentFrames.length;
        
        this.startAnimation(config.animationSpeed, config.loopAnimation);
      } else {
        // Single frame
        const overlay = this.overlayFrames.length === 1 ? this.overlayFrames[0] : null;
  ...
  getConfig() {
    return {
      ...
      errorCorrection: document.querySelector('input[name="errorCorrection"]:checked')?.value || 'Q',
      ...
      overlayIntensity: parseInt(document.getElementById('overlayIntensity').value) || 50,
      ...
      preserveFinders: document.getElementById('preserveFinders')?.checked ?? true,
      animationSpeed: parseInt(document.getElementById('animationSpeed').value) || 100,
      loopAnimation: document.getElementById('loopAnimation')?.checked ?? true,
      reverseAnimation: document.getElementById('reverseAnimation')?.checked ?? false,
      outputWidth: parseInt(document.getElementById('outputWidth').value) || 400,
      outputHeight: parseInt(document.getElementById('outputHeight').value) || 400,
    ...
    reader.onload = async (e) => {
      ...
      if (file.type === 'image/gif') {
        try {
          ...
          this.toast.show(`Loaded ${this.overlayFrames.length} frames`, 'success');
        } catch (err) {
  ...
  resetToDefaults() {
    ...
    // Basic settings
    document.querySelector('input[name="errorCorrection"][value="Q"]').checked = true;
  ...
  loadFromParams(params) {
    ...
    if (params.ec) {
      const radio = document.querySelector(`input[name="errorCorrection"][value="${params.ec}"]`);
    ...
    if (params.style) document.getElementById('moduleStyle').value = params.style;
    if (params.finder) document.getElementById('finderStyle').value = params.finder;
    if (params.mode) document.getElementById('overlayMode').value = params.mode;
    if (params.intensity) document.getElementById('overlayIntensity').value = params.intensity;
    if (params.logoSize) document.getElementById('logoSize').value = params.logoSize;
  ...
  async download() {
    ...
    try {
      ...
      if (this.currentFrames.length > 1) {
        ...
        await this.exporter.downloadSvg(this.currentFrames[this.currentFrameIndex], config);
      } else {
        await this.exporter.downloadImage(this.currentFrames[this.currentFrameIndex], config);
      }
