...
export const examples: PageDefinition = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        "Testing revealed multiple issues: the original code used Error Correction L (minimum redundancy), had a quiet zone of only 2 modules, and was printed at low resolution. Under the café's warm tungsten lighting, the already-faded black modules barely contrasted against the yellowed paper. Older phones struggled; newer phones succeeded but with noticeable delay.",
    ...
    },
    {
      ...
      paragraphs: [
        "Using ANQR, the café owner recreated the code with the identical menu URL but dramatically improved settings: Error Correction H for maximum resilience, a 6-module quiet zone for reliable boundary detection, and brand colours (deep burgundy modules on cream background) that matched the café's interior palette.",
        "A small centre overlay featuring the café's logo was added - kept deliberately subtle to maintain scannability while providing instant brand recognition. Safety Mode confirmed the new design scanned reliably across test devices before any printing.",
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        "The café's configuration demonstrates conservative but effective branding: Error Correction H, 6-module quiet zone, high-contrast brand colours, and a minimal centre overlay. This balance works for most retail counter applications where reliability must come first but brand presence still matters.",
        'Open the generator with these settings pre-loaded and adapt the colours and overlay to your own brand. The key insight: reliability improvements alone can dramatically increase engagement, even before adding any visual styling.',
        'View the related Learn guide on improving existing QR campaigns for the full methodology behind this approach.',
      ],
      images: [
        {
    ...
    // ===== Example 2: Branded Retail Payments (IDs 1071-1097) =====
    {
      ...
      paragraphs: [
        "This example follows a boutique clothing store that had accumulated five different QR codes at their register: bank payment, tips, Instagram, Google reviews, and a hiring link. Each was generated from a different source, printed at different times, and displayed in mismatched stands. The visual chaos was undermining the store's carefully curated aesthetic, and staff regularly dealt with customers scanning the wrong code.",
        'The solution required separating concerns: payment codes needed maximum reliability with conservative styling, while marketing codes could be more expressive. The unified display system that emerged demonstrates how to balance brand presence against the functional demands of different QR use cases.',
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "These four codes were arranged in a branded panel positioned beside (not competing with) the payment code. Clear labels in the store's typography explained each code's purpose. The visual hierarchy was intentional: payment stood alone and prominent; secondary actions grouped together as options.",
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        'The physical display solved several problems simultaneously. Both the payment stand and the secondary panel used matte materials to eliminate glare from overhead lighting. Heights were optimised for customers of varying stature. Angles directed toward the customer queue rather than flat on the counter.',
        'Critically, the store created spare prints from locked ANQR configuration links. When the tips code eventually got coffee-splashed, staff replaced it within minutes using the stored configuration - no screenshots, no guessing at settings, no quality degradation.',
      ],
      images: [
        {
          ...
          caption:
            'Display system blueprint: physical ergonomics optimised for reliable scanning across customer heights and lighting conditions',
        },
      ],
    },
    {
      ...
      paragraphs: [
        "Six weeks after deployment, the store documented significant improvements: zero customer complaints about 'wrong code' confusion, faster transaction times, and a 3x increase in tip submissions (customers could now find and scan the tip code without awkwardly asking staff). Google reviews also increased as the dedicated, well-labeled QR removed friction.",
        "Staff training became simpler: 'payment code is the big one, everything else is on the panel.' When payment apps updated and one briefly showed slower scanning, the conservative payment code design meant it still worked - just marginally slower rather than failing entirely.",
      ...
      images: [
        {
          ...
          caption:
            'Six weeks of operational data demonstrating the business impact of a thoughtful multi-QR display system',
        },
      ],
      links: [
        {
    ...
    // ===== Example 3: Print Scaling (IDs 1098-1124) =====
    {
      ...
      paragraphs: [
        'This example follows an event production company preparing for a major conference. They needed the same QR code - linking to the event app - deployed across radically different formats: attendee badge inserts (3cm), table tent cards (8cm), registration desk signage (30cm), wayfinding posters (60cm), and a massive stage backdrop banner (4 metres). Each format had different viewing distances, lighting conditions, and production workflows.',
    ...
    },
    {
      ...
      paragraphs: [
        'For this event, the company established a strict protocol: one master QR generated in ANQR with optimal settings (Error Correction H, generous quiet zone, clean styling), exported as SVG, and stored as the single authoritative source. The ANQR configuration link was documented alongside the SVG file so the code could be regenerated if needed.',
        'Every designer and vendor received the same SVG master with explicit instructions: place at required size, do not modify, maintain quiet zone clearance. For vendors requiring raster formats, the company provided pre-rendered PNGs at specific sizes with clear naming conventions indicating intended use.',
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        "Each format required specific attention. Badge inserts at 3cm needed the QR to occupy maximum available space - scanning distance would be arm's length. Table tents at 8cm could include decorative framing outside the quiet zone. The 4-metre banner required calculation: from typical audience distance (15-20 metres), modules needed to be clearly distinguishable by phone cameras, which meant the QR needed to be at least 80cm within the banner design.",
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'Zero scanning complaints were logged across 2,000+ attendees. The operational win was equally significant: when a last-minute sponsor addition required new signage, the production team generated it from the master SVG in minutes, confident it would match everything else.',
    ...
    // ===== Example 4: Animated Signage (IDs 1125-1151) =====
    {
      ...
      paragraphs: [
        "This example follows an airline lounge that had been using static QR codes on their digital welcome screens. The screens displayed rotating promotional content, but the QR code for lounge check-in sat in a corner, static and easily overlooked. Analytics showed only 15% of eligible guests used the QR check-in despite it being faster than the desk queue. Most guests simply didn't notice it.",
        'The hypothesis was simple: in a visually busy environment with moving content, a static QR becomes invisible. The solution needed to make the QR noticeable without compromising the reliability required for a check-in flow where failure would mean frustrated travellers and longer desk queues.',
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'Safety Mode validation confirmed every frame remained scannable. Additional testing on the actual LED panels revealed that the pulse needed to be even more subtle than desktop previews suggested - LED brightness and viewing angles affected perceived contrast more than expected.',
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        "The animated QR was positioned in a dedicated 'stable zone' of the screen layout - an area that remained constant while promotional content rotated in the main display area. This separation was crucial: the QR needed visual stability for scanning even while attracting attention through its subtle animation.",
        "A clear call-to-action was added adjacent to the QR: 'Skip the queue - scan to check in.' The text remained static while the QR pulsed, creating a visual hierarchy that drew the eye to the scanning opportunity without overwhelming the promotional content.",
      ],
      images: [
        {
    ...
    },
    {
      ...
      paragraphs: [
        ...
        'See the Learn guide on animated QR codes for digital signage for the technical principles behind safe animation design.',
      ],
      images: [
        {
    ...
    // ===== Example 5: Cross-Lingual Collaboration (IDs 1152-1178) =====
    {
      ...
      paragraphs: [
        "This example follows a consumer electronics company launching a new product simultaneously in 12 markets across 8 languages. Each regional marketing team needed to produce packaging, retail displays, and promotional materials featuring QR codes linking to localised product pages. Previous launches had resulted in inconsistent QR styling, occasional scanning failures, and a 'telephone game' of configuration drift as each team recreated codes from screenshots.",
        "The solution leveraged ANQR's shareable configuration links to establish a single source of truth that every regional team could access, regardless of their interface language. The QR payload used a smart redirect that detected user language, so one code worked globally while delivering localised experiences.",
      ],
      images: [
        {
          ...
          caption:
            'The challenge: 12 markets, 8 languages, and a history of inconsistent QR implementations across regions',
        },
      ],
    },
    {
      ...
      paragraphs: [
        "The global brand team created the authoritative QR configuration in ANQR: brand colours matching the product line's visual identity, Error Correction H for reliability across all print and digital applications, and styling that would reproduce consistently regardless of local production methods. The configuration was locked and the share link documented in the global campaign brief.",
        'Critically, the encoded URL used a language-detecting redirect service. When scanned, users were automatically routed to their local language product page. This meant all 12 markets could use identical QR codes - no per-market payload variations that could introduce errors.',
      ],
      images: [
        {
          ...
          caption:
            'The master configuration: global brand standards encoded into a single, shareable source of truth',
        },
      ],
    },
    {
    ...
    },
    {
      ...
      paragraphs: [
        'Product launch day saw QR codes deployed across packaging, retail displays, event materials, and digital campaigns in all 12 markets simultaneously. Quality audits confirmed visual consistency across regions - the QR on Tokyo retail displays matched the QR on São Paulo packaging matched the QR on Berlin event banners.',
