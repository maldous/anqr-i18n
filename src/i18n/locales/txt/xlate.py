...
def write_txt_file(filepath: Path, mapping: Dict[int, str]) -> None:
    """Write deterministically (sorted by id), atomically."""
...
class LibreTranslateEngine(Engine):
    ...
    def __init__(self) -> None:
        ...
        self._langs_map: dict[str, str] = {}

    def available(self) -> bool:
        return bool(os.environ.get("LIBRETRANSLATE_URL"))
    def normalize_target(self, target: str) -> str:
        """Map on-disk locale codes to this server's supported target code.

        Your server exposes targets like: nb, pt-BR, zh-Hans, zh-Hant (see /languages).
        We return the server's canonical code (preserving case) when possible.
        """
        raw = target.replace("_", "-").strip()
        raw_l = raw.lower()

        # Lazy load /languages once
        ...
        def pick(cand_l: str) -> str | None:
            if not self._langs_map:
                return None
            return self._langs_map.get(cand_l)

        # Chinese: map zh-CN/zh-SG/etc -> zh-Hans, zh-TW/HK/MO -> zh-Hant
        if raw_l == "zh" or raw_l.startswith("zh-"):
            ...
            if got:
                return got
            # Fallback: any zh* target
            if self._langs_map:
                for k, v in self._langs_map.items():
                    if k.startswith("zh"):
                        return v

        # Candidates in priority order
        candidates: list[str] = [raw_l, raw_l.replace("-", "_")]
        base = raw_l.split("-")[0]
        ...
        for a in aliases.get(base, []):
            a_l = a.lower()
            candidates.append(a_l)
            candidates.append(a_l.replace("-", "_"))
        ...
        if self._langs_map:
            ...
            for k, v in sorted(self._langs_map.items()):
                if k.startswith(base):
                    return v

        # If /languages couldn't be fetched, fall back to base code.
        return base
    def _fetch_languages(self) -> set[str]:
        ...
        r.raise_for_status()
        data = r.json()
        # This server returns entries like:
        #   {"code":"en","name":"English","targets":[...]}
        # We translate FROM English, so the authoritative supported target list is the EN entry's targets.
        targets: list[str] = []
        for item in data:
    ...
    def supports_target(self, target: str) -> bool:
        # normalize_target() lazy-loads /languages; then we check against the server target set.
        t = self.normalize_target(target)
        if not getattr(self, "_langs_map", None):
            return True
        return t.lower() in self._langs_map
        # Keep canonical case from server, but index case-insensitively
        self._langs_map = {str(t).lower(): str(t) for t in targets}
        return set(self._langs_map.values())
...
def process_language_file(
    ...
    if used_engine == "unsupported":
        ...
        # Remove 0-byte placeholders from previous runs so they don't look "done".
        try:
