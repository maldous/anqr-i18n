export class QRGenerator {
  ...
  async generate(config, overlayCanvas = null) {
    ...
    if (config.overlayMode === 'subpixel' && overlayCanvas) {
      return this.generateSubpixelQR(qr, config, overlayCanvas, moduleCount, false);
    }
    if (config.overlayMode === 'subpixel-size' && overlayCanvas) {
      return this.generateSubpixelQR(qr, config, overlayCanvas, moduleCount, true);
    }
    ...
    if (overlayCanvas) {
      overlayData = this.getOverlayData(overlayCanvas, moduleCount, config.colorMode || 'color', config.invertImage || false);
    }
    ...
    for (let row = 0; row < moduleCount; row++) {
      for (let col = 0; col < moduleCount; col++) {
        ...
        const applyOverlay = overlayData && (!config.preserveFinders || !isFinder);
          ...
          switch (config.overlayMode) {
            case 'halftone':
              if (isDark) {
                ...
                const minSize = 0.3;
                const maxSize = 1.0;
                moduleSizeModifier = minSize + (1 - brightness) * (maxSize - minSize) * intensity + 
            ...
            case 'blend':
              if (isDark) {
                ...
                if (blendColor) {
                  ...
                  moduleColor = this.blendColors(config.fgColor, blendColor, blendAmount);
                }
              }
              break;
            ...
            case 'gapfill':
              ...
              if (isDark) {
                ...
                if (gapColor) {
                  ...
                  moduleColor = this.blendColors(config.bgColor, gapColor, fadeAmount);
                  shouldDrawCell = true;
                }
              }
              break;
            ...
            case 'pixelate':
              ...
              if (isDark) {
                ...
                if (pixCount > 0) {
                  ...
                  moduleColor = this.blendColors(config.fgColor, blockColor, pixBlendAmt);
                }
              }
              break;
            ...
            case 'outline':
              if (isDark) {
                ...
                if (outlineColor) {
                  ...
                  moduleColor = this.blendColors(config.fgColor, outlineColor, outlineBlend);
                }
              }
              break;
        ...
        if (shouldDrawCell && moduleOpacity > 0) {
          ...
          const gap = config.moduleGap ? (moduleSize * config.moduleGap / 100) : 0;
  ...
  applyTrueDither(qr, overlayData, config, extremeMode = false) {
    ...
    if (extremeMode) {
      ...
      const ecCapacity = { 'L': 0.07, 'M': 0.15, 'Q': 0.25, 'H': 0.30 };
  ...
  drawModule(ctx, x, y, size, style) {
    ...
    switch (style) {
      case 'rounded':
        this.roundRect(ctx, x + padding, y + padding, innerSize, innerSize, size * 0.3);
        ctx.fill();
        break;
   ...
   */
  generateSubpixelQR(qr, config, overlayCanvas, moduleCount, useHalftoneCenter = false) {
    ...
    const overlayData = this.getSubpixelOverlayData(overlayCanvas, moduleCount * subpixelSize, config.colorMode || 'color');
            ...
            if (subRow === 1 && subCol === 1) {
              ...
              if (overlayColor && intensity > 0) {
                ...
                if (intensity >= 1) {
                  ...
                  ctx.fillStyle = this.blendColors('#808080', overlayColor, intensity);
                }
              } else {
