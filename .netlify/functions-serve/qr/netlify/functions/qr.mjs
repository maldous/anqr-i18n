...
function errorDiffusion(imageData, width, height, kernelName = "floyd_steinberg", serpentine = true, levels = 2, strength = 100, colorMode = "bw") {
...
function blueNoiseDither(imageData, width, height, tileSize = 64, seed = 0, levels = 2, strength = 100, colorMode = "bw") {
...
function whiteNoiseDither(imageData, width, height, seed = 0, strength = 100, colorMode = "bw") {
  const rng = createRNG(seed);
  const strengthFactor = strength / 100;
  const resultMatrix = [];
  const resultColors = [];
...
function gaussianNoiseDither(imageData, width, height, seed = 0, strength = 100, colorMode = "bw") {
  const rng = createRNG(seed);
  const strengthFactor = strength / 100;
...
function triangularNoiseDither(imageData, width, height, seed = 0, strength = 100, colorMode = "bw") {
  const rng = createRNG(seed);
  const strengthFactor = strength / 100;
...
var QRGenerator = class {
  ...
  async generate(config, overlayCanvas = null) {
    ...
    if ((config.overlayMode === "dither" || config.overlayMode === "extreme") && overlayData) {
      const useAdvancedDither = config.ditherKind && config.ditherKind !== "true_dither" && config.ditherKind !== "error_diffusion";
    ...
    for (let row = 0; row < moduleCount; row++) {
      for (let col = 0; col < moduleCount; col++) {
        ...
        const applyOverlay = overlayData && (!config.preserveFinders || !isFinder) && (!config.preserveTiming || !isTiming) && (!config.preserveAlignment || !isInAlignmentSet) && (!protectFormat || !isFormatInfo) && (!protectVersion || !isVersionInfo);
  ...
  _hasPreprocessingOptions(config) {
    return config.overlayBrightness && config.overlayBrightness !== 0 || config.overlayContrast && config.overlayContrast !== 0 || config.overlayGamma && config.overlayGamma !== 1 || config.overlaySaturation && config.overlaySaturation !== 0 || config.overlayHueRotate && config.overlayHueRotate !== 0 || config.invertImage || config.overlayBlur && config.overlayBlur > 0 || config.overlaySharpen && config.overlaySharpen > 0 || config.overlayPosterize && config.overlayPosterize > 0 || config.overlayEdgeDetect && config.overlayEdgeDetect !== "off" || config.overlayThreshold !== void 0 && config.overlayThreshold !== 128 || config.overlayFlipX || config.overlayFlipY || config.overlayRotate && config.overlayRotate !== 0;
  }
  /**
  ...
  async _preprocessOverlay(overlayCanvas, config) {
    ...
    if (config.overlayThreshold !== void 0 && config.overlayThreshold !== 128 && filterOptions.edgeDetect === "off") {
  ...
  _applyFiltersManual(imageData, options) {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      if (options.hueRotate && options.hueRotate !== 0) {
        ...
        const newG = r * (0.213 - cos * 0.213 + sin * 0.143) + g * (0.715 + cos * 0.285 + sin * 0.14) + b * (0.072 - cos * 0.072 - sin * 0.283);
  ...
  _applyPerModuleColor(mode, baseColor, overlayData, row, col, moduleCount, config) {
    ...
    switch (mode) {
      ...
      case "by_cluster": {
        ...
        const cos = Math.cos(angle);
        const sin = Math.sin(angle);
        const r = Math.max(0, Math.min(255, parsed.r * (0.213 + cos * 0.787 - sin * 0.213) + parsed.g * (0.715 - cos * 0.715 - sin * 0.715) + parsed.b * (0.072 - cos * 0.072 + sin * 0.928)));
        const g = Math.max(0, Math.min(255, parsed.r * (0.213 - cos * 0.213 + sin * 0.143) + parsed.g * (0.715 + cos * 0.285 + sin * 0.14) + parsed.b * (0.072 - cos * 0.072 - sin * 0.283)));
        const b = Math.max(0, Math.min(255, parsed.r * (0.213 - cos * 0.213 - sin * 0.787) + parsed.g * (0.715 - cos * 0.715 + sin * 0.715) + parsed.b * (0.072 + cos * 0.928 + sin * 0.072)));
...
function getWatermarkPositions(position, canvasWidth, canvasHeight, watermarkWidth, watermarkHeight, margin = 10) {
  const positions = [];
  switch (position) {
    case "center":
      positions.push({
...
function generatePatternFramesServer(sourceCanvas, pattern, frameCount = 24, seed = 0, easing = "linear") {
  if (pattern === "none") {
    return [sourceCanvas];
  }
  const frames = [];
...
async function fetchImageWithBuffer(url) {
  try {
    ...
    const validImageTypes = ["image/png", "image/jpeg", "image/gif", "image/webp", "image/svg+xml", "image/bmp"];
...
var qr_default = async (request) => {
  ...
  const eccAwareRiskBudget = Math.min(100, Math.max(0, parseInt(params.get("eccRisk") || "50", 10)));
  ...
  const overlayBrightness = Math.min(100, Math.max(-100, parseInt(params.get("brightness") || "0", 10)));
  const overlayContrast = Math.min(100, Math.max(-100, parseInt(params.get("contrast") || "0", 10)));
  const overlayGamma = Math.max(0.1, Math.min(3, parseFloat(params.get("gamma") || "1")));
  const overlaySaturation = Math.min(100, Math.max(-100, parseInt(params.get("saturation") || "0", 10)));
  ...
  const overlayThreshold = Math.min(255, Math.max(0, parseInt(params.get("threshold") || "128", 10)));
  ...
  const palette = paletteParam ? decodeURIComponent(paletteParam).split(",").map((c) => c.startsWith("#") ? c : `#${c}`) : void 0;
  const paletteMode = params.get("paletteMode") || "position";
  const maxOverlayIntensityL = parseInt(params.get("maxIntL") || params.get("maxOverlayIntensityL") || "100", 10);
  const maxOverlayIntensityM = parseInt(params.get("maxIntM") || params.get("maxOverlayIntensityM") || "100", 10);
  ...
  const animationFrames = Math.min(60, Math.max(1, isNaN(animationFramesRaw) ? 24 : animationFramesRaw));
  const animationSpeedRaw = parseInt(params.get("animSpeed") || "100", 10);
  const animationSpeed = Math.min(1e3, Math.max(10, isNaN(animationSpeedRaw) ? 100 : animationSpeedRaw));
  ...
  const metaAuthor = params.get("metaAuthor") ? decodeURIComponent(params.get("metaAuthor")) : void 0;
  const metaCopyright = params.get("metaCopy") ? decodeURIComponent(params.get("metaCopy")) : void 0;
  const metaDescription = params.get("metaDesc") ? decodeURIComponent(params.get("metaDesc")) : void 0;
  ...
  try {
    ...
    if (watermarkEnabled && (watermarkKind === "image" || watermarkKind === "pattern") && watermarkImageUrl) {
    ...
    const metadata = metaTitle || metaAuthor || metaCopyright || metaDescription ? {
      title: metaTitle,
      author: metaAuthor,
